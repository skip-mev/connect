// Code generated by mockery v2.21.1. DO NOT EDIT.

package mocks

import (
	context "context"

	oracletypes "github.com/skip-mev/slinky/x/oracle/types"
	mock "github.com/stretchr/testify/mock"

	time "time"

	uint256 "github.com/holiman/uint256"
)

// Oracle is an autogenerated mock type for the Oracle type
type Oracle struct {
	mock.Mock
}

// GetLastSyncTime provides a mock function with given fields:
func (_m *Oracle) GetLastSyncTime() time.Time {
	ret := _m.Called()

	var r0 time.Time
	if rf, ok := ret.Get(0).(func() time.Time); ok {
		r0 = rf()
	} else {
		r0 = ret.Get(0).(time.Time)
	}

	return r0
}

// GetPrices provides a mock function with given fields:
func (_m *Oracle) GetPrices() map[oracletypes.CurrencyPair]*uint256.Int {
	ret := _m.Called()

	var r0 map[oracletypes.CurrencyPair]*uint256.Int
	if rf, ok := ret.Get(0).(func() map[oracletypes.CurrencyPair]*uint256.Int); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(map[oracletypes.CurrencyPair]*uint256.Int)
		}
	}

	return r0
}

// IsRunning provides a mock function with given fields:
func (_m *Oracle) IsRunning() bool {
	ret := _m.Called()

	var r0 bool
	if rf, ok := ret.Get(0).(func() bool); ok {
		r0 = rf()
	} else {
		r0 = ret.Get(0).(bool)
	}

	return r0
}

// Start provides a mock function with given fields: ctx
func (_m *Oracle) Start(ctx context.Context) error {
	ret := _m.Called(ctx)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context) error); ok {
		r0 = rf(ctx)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// Stop provides a mock function with given fields:
func (_m *Oracle) Stop() {
	_m.Called()
}

type mockConstructorTestingTNewOracle interface {
	mock.TestingT
	Cleanup(func())
}

// NewOracle creates a new instance of Oracle. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewOracle(t mockConstructorTestingTNewOracle) *Oracle {
	mock := &Oracle{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
