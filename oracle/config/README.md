...
> can be generated by the `make update-local-config` command. The generation code can be found in [generate_toml.go](../../config/local/generate_toml.go). Note that chain's that support the oracle module (`x/oracle`) should provide a predefined oracle side-car configuration file. Validators **should not** need to create their own oracle side-car configuration file.
> 
