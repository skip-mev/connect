...
// HashCurrencyPairStrategy is a strategy that uses the hash of the currency pair as the unique ID
// for a given currency pair and utilizes raw prices stored in the x/oracle state as the price
...
// ID returns the ID of the given currency pair, by taking the hash of the currency pair and using
// that as the ID.
...
// FromID returns the currency pair with the given ID, it first checks the cache for the currency pair
// and if it is not found in the cache, it will attempt to retrieve the currency pair from the x/oracle
// state.
func (s *HashCurrencyPairStrategy) FromID(ctx sdk.Context, id uint64) (slinkytypes.CurrencyPair, error) {
	...
	// if the currency pair is not found in the cache, attempt to retrieve it from the x/oracle state by
	// populating the cache with all currency pairs.
...
// CurrencyPairToHashID returns the ID of the given currency pair, by taking the sha256 hash of the
// currency pair.
