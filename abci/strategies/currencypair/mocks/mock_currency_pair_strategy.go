...
// GetMaxErrorAmount provides a mock function with given fields: ctx
func (_m *CurrencyPairStrategy) GetMaxErrorAmount(ctx types.Context) (uint64, error) {
	ret := _m.Called(ctx)

	if len(ret) == 0 {
		panic("no return value specified for GetMaxErrorAmount")
	}
