...
docker-build:
	...
	@DOCKER_BUILDKIT=1 $(DOCKER) build -t skip-mev/connect-e2e:local -f contrib/images/connect.e2e.Dockerfile .
	@DOCKER_BUILDKIT=1 $(DOCKER) build -t skip-mev/connect-e2e-oracle:local -f contrib/images/connect.sidecar.prod.Dockerfile .
