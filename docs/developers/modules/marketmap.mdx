...
The `Marketmap` module's Keeper is instantiated with an `Authority` address, guarding the mutation of the `Marketmap` by a specific address. This address can be the `governance` address, allowing governance propositions to mutate the `Marketmap`.

The `Authority` of the `Marketmap` module can modify the Keeper's `MarketAuthorities`. This allows the `Authority` to add or remove addresses that are authorized to update the `Marketmap`.
...
Proto definitions for all types, queries, and messages can be found [here](https://github.com/skip-mev/slinky/tree/main/proto/slinky/marketmap).
