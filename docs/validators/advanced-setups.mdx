...
### Connect Flags
In addition to the setup in the [quickstart](quickstart), you will need to pass some additional flags when booting up the Connect instance.

```shell
slinky <... your other flags...> --marketmap-provider dydx_api --market-map-endpoint https://<dydx-node REST API endpoint>
```

The markets dYdX provides for also require API keys. If you have not received API keys, please reach out in the relevant communication channels. Please see [API Keys](configuration#api-keys) to learn how to configure them in Connect.
