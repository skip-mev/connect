...
This document serves as a reference for Connect's configurable values.
...
Please remember to change the `oracle_address` in your application's `app.toml` if you change any of the values below.
...
| `--marketmap-provider`           | `marketmap_api` | MarketMap provider to use (marketmap_api, dydx_api, dydx_migration_api).                      |
| `--oracle-config`                | `""`            | Overrides the Oracle Configuration.                                                           |
...
Some chains include markets that require API keys to provide data for. In order to do this, you will need to utilize the `--oracle-config` flag and pass in a `json` file that looks like the following:
