...
func TestHandleMessage(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			resp: providertypes.GetResponse[mmtypes.Ticker, *big.Int]{
				Resolved: map[mmtypes.Ticker]providertypes.Result[*big.Int]{
					constants.BITCOIN_USD: providertypes.NewResult[*big.Int](big.NewInt(4206900000000), time.Now()),
				},
		...
		},
		{
			...
			resp: providertypes.GetResponse[mmtypes.Ticker, *big.Int]{
				Resolved: map[mmtypes.Ticker]providertypes.Result[*big.Int]{
					constants.BITCOIN_USD:  providertypes.NewResult[*big.Int](big.NewInt(4206900000000), time.Now()),
					constants.ETHEREUM_USD: providertypes.NewResult[*big.Int](big.NewInt(200000000000), time.Now()),
					constants.SOLANA_USD:   providertypes.NewResult[*big.Int](big.NewInt(100000000000), time.Now()),
				},
				UnResolved: map[mmtypes.Ticker]error{},
			},
			expUpdateMsg: func() []handlers.WebsocketEncodedMessage {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			wsHandler, err := cryptodotcom.NewWebSocketDataHandler(logger, cryptodotcom.DefaultMarketConfig, cryptodotcom.DefaultWebSocketConfig)
			require.NoError(t, err)
