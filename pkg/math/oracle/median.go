...
func (m *MedianAggregator) AggregateFn() aggregator.AggregateFn[string, map[oracletypes.CurrencyPair]*big.Int] {
	return func(
		...
		for cp, price := range feedMedians {
			scaledPrice, err := ScaleUpCurrencyPairPrice(cp.Decimals, price)
			if err != nil {
				m.logger.Error(
		...
		// Scale all aggregated medians back to the original number of decimals.
		for cp, price := range aggregatedMedians {
			unscaledPrice, err := ScaleDownCurrencyPairPrice(cp.Decimals, price)
			if err != nil {
				m.logger.Error(
					"failed to scale price",
					zap.Error(err),
