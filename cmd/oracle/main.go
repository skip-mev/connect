...
func main() {
	...
	marketCfg, err := config.ReadMarketConfigFromFile(*marketCfgPath)
	if err != nil {
	...
	apiFactory := oraclefactory.APIQueryHandlerFactory(marketCfg)      // Replace with custom API factory.
	wsFactory := oraclefactory.WebSocketQueryHandlerFactory(marketCfg) // Replace with custom websocket factory.
